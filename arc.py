...
def process_arc_file(arc_folder, arc_file, location_id, new_item_id):
    ...
    xml_file_path = os.path.join(unpack_folder, 'stage', unpack_folder, f'soft\{unpack_folder}_item.lot.xml')
...
def update_item_ids(input_file, arc_folder, mapping_file):
    ...
    try:
        ...
        # Step 5: Repack all unpacked .arc files
        logging.info("Repacking all unpacked .arc files...")
        for folder in os.listdir(arc_folder):
            if os.path.isdir(folder) and folder != "Archive":
                subprocess.run(['pc-re5.bat', folder], check=True)
                logging.info(f"Repacked {folder} back into its .arc file.")
